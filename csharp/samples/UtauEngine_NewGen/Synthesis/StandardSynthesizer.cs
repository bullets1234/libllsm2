    ...
    public sealed class StandardSynthesizer
    {
        ...
        public SynthesisResult Synthesize(ChunkHandle srcChunk, int fs, SynthesisParams p)
        {
            ...
            using (_diag.Profiler.Measure("to_layer1"))
            {
                ...
                bool residualCorrectionEnabled = Environment.GetEnvironmentVariable("L2R_RESIDUAL") != "0";
                ...
                if (Environment.GetEnvironmentVariable("L2R_SMOOTH") != "0")
                {
            ...
            var dstChunk = LlsmBindings.Llsm.CreateChunk(confCopy, 0);
            ...
            log.Info(Stage, $"Pitch shift {srcF0:F1}->{targetF0:F1}Hz (ratio={pitchShiftRatio:F4}), VtmagnComp={vtmagnCompensation:F2}dB, {dstNfrm} frames");
                ...
                for (int i = 0; i < dstNfrm; i++)
                {
                    ...
                    if (originalF0 > 0)
                    {
                        ...
                        if (newF0 > originalF0 * 1.1f)
                            FrameInterpolator.ExtendVsphseForPitchShift(newFramePtr, originalF0, newF0, fs);
            ...
            var growl = new GrowlEffect(p.GrowlStrength, log);
            ...
            return Render(dstChunk, fs, p.UseOversampling, useLayer1Synthesis, log);
        }
